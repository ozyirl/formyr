...
const clsx = (...args: ClassValue[]) => args.filter(Boolean).join(" ");
...
const StopButton = memo(
	PureStopButton,
	(prev, next) => prev.onStop === next.onStop
);
...
const SendButton = memo(PureSendButton, (prevProps, nextProps) => {
	...
	if (prevProps.attachments.length !== nextProps.attachments.length)
		return false;
	if (
		prevProps.attachments.length > 0 &&
		...
		return new Promise((resolve) => {
			setTimeout(() => {
				try {
					...
					console.error(
						"MOCK: Failed to create object URL for preview:",
						error
					);
					resolve(undefined);
				} finally {
					...
					setUploadQueue((currentQueue) =>
						currentQueue.filter((name) => name !== file.name)
					);
				}
			}, 700); // Simulate delay
		});
	...
	const handleFileChange = useCallback(
		async (event: ChangeEvent<HTMLInputElement>) => {
			...
			if (invalidFiles.length > 0) {
				console.warn(
					`Skipped ${invalidFiles.length} files larger than ${
				...
				setUploadQueue((currentQueue) =>
					currentQueue.filter(
						(name) => !invalidFiles.some((f) => f.name === name)
					)
				);
			}
